...
{
    public partial class AppliedTeacherForm : Form
    {
        static TrungTamThayThanhContext db = new TrungTamThayThanhContext();
        public AppliedTeacherForm()
        {
            InitializeComponent();
        }
        ...
        {
            if (IDNumberTxb.Text == String.Empty||IDNumberTxb.Text.Length>12)
            {
                MessageBox.Show("Số chứng minh nhân dân/thẻ căn cước phải có độ dài 12 ký tự trở xuống và không được để trống!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (FullNameTxb.Text == String.Empty)
            {
                MessageBox.Show("Tên giáo viên không được để trống! Xin vui lòng nhập tên giáo viên!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (PhoneNumberTxb.Text == String.Empty)
            {
                MessageBox.Show("Số điện thoại giáo viên không được để trống! Xin vui lòng nhập số điện thoại!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            bool gt = false;
            ...
            var giaovien = new GiaoVien()
            {
                ...
                TenGV = FullNameTxb.Text,
                NgaySinh = DateBirthDTP.Value,
                SoDT = PhoneNumberTxb.Text,
                SoNamKinhNghiem = int.Parse(ExperienceNumUD.Value.ToString()),
                GioiTinh = gt,
                MonHoc = SubjectListCbx.SelectedValue.ToString()
            };
            ...
            {
                MessageBox.Show("Không thêm được giáo viên! Số CMND này đã tồn tại!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void AdjustDatagridHeight()
        {
            ...
            if (TeacherListDgv.Height < 600)
            {
                ...
                DataPanel.Height = totalRowHeight + 10;
                this.Height = DataPanel.Height + 170;
                TeacherListDgv.Width = DataPanel.Width;
        ...
        {
            MessageBox.Show("Vui lòng nhập các thông tin sau đây cho hợp đồng!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            PendingContractForm f = new PendingContractForm();
            f.IDNumberTxb.Text = this.IDNumberTxb.Text;
            f.ShowDialog();
            InitializeList();
        }
